...
class FavouritesView(ListView):
    ...
    def get_queryset(self):
        favorite_books = FavoriteBook.objects.filter(user=self.request.user).select_related('book')
        self.latest_book = favorite_books.last()
        self.other_books = favorite_books.exclude(id=self.latest_book.id) if self.latest_book else []
        return favorite_books

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['latest_book'] = self.latest_book
        context['other_books'] = self.other_books
        return context
